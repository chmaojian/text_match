...
if __name__ == '__main__':
    ...
    csv_file_name = path.join("train_data.csv")
    reader = tf.contrib.timeseries.CSVReader(
        csv_file_name,
        column_names=((tf.contrib.timeseries.TrainEvalFeatures.TIMES,)
                      + (tf.contrib.timeseries.TrainEvalFeatures.VALUES,) * 12))
    train_input_fn = tf.contrib.timeseries.RandomWindowInputFn(
        reader, batch_size=4, window_size=32)

    estimator = ts_estimators.TimeSeriesRegressor(
        model=_LSTMModel(num_features=12, num_units=128),
    ...
    print(observed)
    print(predicted)
    # error_lines = plt.plot(observed_times, abs(observed - evaluated), label="error", color="b")
